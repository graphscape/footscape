...
public class ProxyServlet extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			copyStream(inr, out, -1);// request data
			out.flush();
			...
			Integer len = treatHeader(in, req, res);
			if (len != null) {
				log("+ copyStream");
				// if ( debugFlag ) res.setContentType("text/plain");
				out = res.getOutputStream();
				copyStream(in, out, len);// response data.
			} else
				log("- copyStream");
		} catch (IOException e) {
			log("out-in.open!");
	 ...
	 */
	private Integer treatHeader(InputStream in, HttpServletRequest req,
	 ...
	 * @throws IOException
	 */
	public void copyStream(InputStream in, OutputStream out, int len)
			throws IOException {
		StringBuffer sb = new StringBuffer();
		sb.append("\nstart of copyStream:----------------------\n");
		this.copyStream(in, out, len, sb);
		sb.append("\nend of copyStream.....................\n");
		this.log(sb.toString());
	}

	public void copyStream(InputStream in, OutputStream out, int len,
			StringBuffer debug) throws IOException {
