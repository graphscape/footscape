...
public class BondHandler implements EventHandlerI<EndpointBondEvent> {
	...
	public void handle(EndpointBondEvent t) {
		...
		this.control.getLazy(MainControlI.LZ_ACTIVITIES, true);
		// active search
		this.control.getLazy(MainControlI.LZ_EXP_SEARCH, true);
		// active user exp list
		this.control.getLazy(MainControlI.LZ_UE_LIST, true);
		// active cooper control.
		this.control.getLazy(MainControlI.LZ_COOPER, true);

		this.control.getLazy(MainControlI.LZ_ACHAT, true);
	}
